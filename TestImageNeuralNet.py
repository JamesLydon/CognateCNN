...
# each language. "en, es, de, it" are the abbreviations Wikipedia uses. I chose to just scrape the Wikipedia page
# "Wikipedia" to serve as a single lengthy page on all language versions of the site.
languages = {
    'en': ['Wikipedia'],
    'es': ['Wikipedia'],
    'de': ['Wikipedia'],
...
def create_neural_data():
    ...
    Path("testimages/en").mkdir(parents=True, exist_ok=True)
    Path("testimages/es").mkdir(parents=True, exist_ok=True)
    Path("testimages/de").mkdir(parents=True, exist_ok=True)
    Path("testimages/it").mkdir(parents=True, exist_ok=True)
...
def convert_dict_to_images(dic, maxletters, tag):
    for word in dic:
        if len(word) <= maxletters:
            ...
            imgfilename = "testimages/" + str(tag) + "/" + wordstring + ".png"
...
def neural_learning():
    ...
    image_count = sum(len(files) for _, _, files in os.walk(r'testimages'))
    BATCH_SIZE = 32
    IMG_HEIGHT = 84
    IMG_WIDTH = 84
